...
from backend.models.fraud_scan import (
    LocalisationDetail,
    RegulatoryDetail,
    ScamCategory,
    ScamDetail,
...
from backend.services.fraud_scan.prompts import SYSTEM_PROMPT, build_user_prompt
...
class FraudDetector:
    ...
    async def _call_with_retry(self, prompt: str, ref: str, attempt: int = 0) -> str | None:
        kwargs: dict = {
            "model": self._model,
            "messages": [
        ...
        try:
            ...
            if self._reasoning_effort and attempt == 0:
                logger.warning("BadRequestError (unsupported extra_body), retrying without reasoning: %s", exc)
    ...
    def _parse(self, raw: str) -> ClassificationResult | None:
        ...
        try:
            ...
            try:
                category = ScamCategory(str(scam_type_raw.get("category", "UNKNOWN")).upper())
            except ValueError:
                category = ScamCategory.UNKNOWN
            scam_type_confidence = max(0.0, min(1.0, float(scam_type_raw.get("confidence", 0.0))))
