"""Loads user + transaction data from S3 into memory on startup.

Mutations from simulate_transaction happen in-memory only (not persisted to S3).

Expects:
  s3://{S3_DATA_BUCKET}/data/users.json
...
from backend.models.behavioral import Transaction, User
