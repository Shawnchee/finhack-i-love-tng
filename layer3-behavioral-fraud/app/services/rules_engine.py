"""Rule-based fraud checks. Each rule inspects the candidate transaction
against the user's historical transactions and emits a ReasonCode when it fires.

Rules (design doc section 4):
...
from app.models.response import ReasonCode, Severity
...
# Baseline = "established" history. Anything newer than this is treated as part
# of the candidate cluster (e.g. rapid-fire scam tx) and excluded from
# avg/max/seen-recipient/typical-hour stats. Velocity + dormancy still see everything.
...
def _user_top_hours(history: list[Transaction], coverage: float) -> set[int]:
    """Smallest set of hours covering `coverage` of user's historical activity."""
    if not history:
        return set()
...
def run_all(txn: Transaction, user: User) -> list[ReasonCode]:
    ...
    # 3. NEW_RECIPIENT_LARGE_AMOUNT (swallows plain NEW_RECIPIENT when it fires)
    ...
    # 6. HIGH_VELOCITY  (count includes candidate transaction itself)
